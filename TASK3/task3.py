...
if __name__ == '__main__':
    ...
    content =  list(array("B",open(sys.argv[1],'r').read()))
    data = str(asm_data(content))
    if(len(content) > 30*1024):
        print "The file is too large"
        sys.exit()
	
    asm_code = (template % {"len":len(content),"sectors":((len(content)-1)/512+1),"data":data,"crc": binascii.crc32(open(sys.argv[1],'r').read())})
